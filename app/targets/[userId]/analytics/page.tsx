...
function StatCard({ value, label, color, sub }: { value: string | number; label: string; color?: string; sub?: string }) {
  ...
  return (
    ...
    >
      ...
      <p className="mt-1 text-xl font-bold leading-tight" style={{ color: c }}>{value}</p>
...
function PresenceTab({ userId }: { userId: string }) {
  ...
  const pieData = (data.sessions || [])
    .map((s) => ({ label: s.status, value: s.total_ms || 0, color: STATUS_COLORS[s.status] || STATUS_COLORS.offline }))
    .filter((d) => d.value > 0)
  ...
  const platformData = (data.platformBreakdown || []).map((p) => ({
    ...
    value: p.total_ms || 0,
    color:
      p.platform === "desktop" ? "var(--color-chart-1)" :
      p.platform === "mobile"  ? "var(--color-status-online)" :
      "var(--color-chart-3)",
  }))
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-5 md:grid-cols-2">
        <Card>
          ...
          <CardContent>
            {pieData.length > 0
              ? <PieChart data={pieData} size={120} formatValue={formatMs} />
      ...
      <Card>
        <CardHeader className="pb-2">
          <CardTitle className="text-sm">Daily Active Minutes — Last 30 Days</CardTitle>
        </CardHeader>
        <CardContent>
  ...
  return (
    <div className="space-y-6">
      <div className="grid gap-3 sm:grid-cols-3">
        ...
        <StatCard value={games.length}                       label="Games Played"  color="var(--color-chart-3)" />
        <StatCard value={`${data.peakGamingHour ?? "—"}:00`} label="Peak Hour"    color="var(--color-chart-4)" />
      </div>

      {data.recentlyStarted?.length > 0 && (
        <div className="flex flex-wrap gap-2">
          <span className="text-xs text-muted-foreground mr-1 self-center">New:</span>
...
function MessagesTab({ userId }: { userId: string }) {
  ...
  const hourData = byHour.map((v: number, i: number) => ({ label: `${i}`, value: v }))
  ...
  return (
    <div className="space-y-6">
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <StatCard value={(data.vocabularyRichness || 0).toFixed(3)}        label="Vocab Richness" color="var(--color-chart-3)" />
      </div>
      <Card>
        <CardHeader className="pb-2"><CardTitle className="text-sm">Messages by Hour of Day</CardTitle></CardHeader>
        <CardContent>
          <LineChart data={hourData} color="var(--color-chart-3)" height={130} formatValue={(v) => `${v}`} />
        </CardContent>
      </Card>
      {heatmapData?.weeklyGrid && (
        <Card>
          ...
          <CardContent>
            <Heatmap data={heatmapData.weeklyGrid.map((row) => row.map((b) => b.eventCount))} color="var(--color-chart-3)" />
          </CardContent>
        </Card>
      )}
    </div>
      ...
      <div className="grid gap-5 md:grid-cols-2">
        {channelData.length > 0 && (
          <Card>
            ...
            <CardContent><BarChart data={channelData} formatValue={(v) => formatMs(v)} /></CardContent>
          </Card>
        )}
        {byHourData.length > 0 && (
          <Card>
    ...
    <div className="space-y-6">
      ...
      {data.recentTrack && (
        <Card
          ...
          style={{ borderColor: "var(--color-spotify)30" }}
        >
          <div
          ...
          >
            ...
            <div>
              ...
              <p className="font-semibold">{data.recentTrack.song}</p>
      ...
      <div className="grid gap-5 md:grid-cols-2">
        {data.topArtists?.length > 0 && (
          <Card>
            ...
            <CardContent>
              <BarChart
                data={data.topArtists.slice(0, 10).map((a) => ({ label: a.name, value: a.listens, color: "var(--color-spotify)" }))}
              />
            </CardContent>
          </Card>
        )}
      ...
      {data.topSongs?.length > 0 && (
        <Card>
          <CardHeader className="pb-2"><CardTitle className="text-sm">Top Songs</CardTitle></CardHeader>
          <CardContent>
            <BarChart
              data={data.topSongs.slice(0, 10).map((s) => ({
  ...
  return (
    <div className="space-y-6">
      <div className="grid gap-3 sm:grid-cols-2">
        <StatCard value={data.connections.length}  label="Connections Found"   color="var(--color-chart-1)" />
        <StatCard value={data.totalInteractions}   label="Total Interactions"  color="var(--color-chart-3)" />
      </div>

      <Card>
        <CardHeader className="pb-2"><CardTitle className="text-sm">Social Graph</CardTitle></CardHeader>
        <CardContent>
          <svg width="100%" height="260" viewBox="0 0 400 260">
            {/* Target node */}
            <circle cx="200" cy="130" r="22" fill="var(--color-primary)" opacity="0.9" />
            <circle cx="200" cy="130" r="22" fill="none" stroke="var(--color-primary)" strokeWidth="2" opacity="0.4" />
            <text x="200" y="134" textAnchor="middle" fill="white" fontSize="10" fontWeight="600">You</text>

            {connections.slice(0, 12).map((conn, i) => {
              const angle  = (i / Math.min(connections.length, 12)) * Math.PI * 2 - Math.PI / 2
              const radius = 72 + (1 - conn.score / maxScore) * 38
              const x      = 200 + Math.cos(angle) * radius
              const y      = 130 + Math.sin(angle) * radius
              const thick  = Math.max(1, (conn.score / maxScore) * 3.5)
              const nodeR  = 7 + (conn.score / maxScore) * 7
              return (
                <g key={i}>
                  <line
                    x1="200" y1="130" x2={x} y2={y}
                    stroke="var(--color-primary)" strokeWidth={thick} opacity="0.35"
                  />
                  ...
                  <text x={x} y={y + 3.5} textAnchor="middle"
                    fill="var(--color-muted-foreground)" fontSize="8"
                  >
                    {conn.userId.slice(-4)}
                  </text>
                </g>
          ...
          {connections.map((conn, i) => (
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="font-mono text-sm font-medium">{conn.userId}</p>
              ...
              <div className="text-right text-xs text-muted-foreground tabular-nums">
                ...
                <p>{conn.messageInteractions}msg · {conn.reactionInteractions}rx</p>
              </div>
            </div>
          ))}
        </CardContent>
